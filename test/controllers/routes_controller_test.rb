...
class RoutesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "find shortest route" do 
    ...
    cases.each do |test_case|
      ...
      if test_case[:status] == 200
        ...
        # get iterative_find_shortest_distance_routes_url, params: test_case.slice(:origin, :destination)
        # assert_equal(test_case[:answer], JSON.parse(response.body)['answer'].to_i, "iterative distance wrong for #{test_case[:origin]} to #{test_case[:destination]}")
  ...
  test "find shortest route via djystras algorithm" do
    ...
    cases.each do |test_case|
      get djystras_algo_for_shortest_path_routes_url, params: test_case.slice(:origin, :destination)
      assert_equal(test_case[:answer], JSON.parse(response.body)['answer'].to_i, "djykstra distace wrong for #{test_case[:origin]} to #{test_case[:destination]}")
